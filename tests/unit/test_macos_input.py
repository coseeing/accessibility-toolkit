...
def test_event_tap_manager_threaded_start_does_not_run_loop_inline(monkeypatch):
    ...
    monkeypatch.setattr("accessibility_toolkit.input.macos.event_tap.threading.Thread", fake_thread)
    monkeypatch.setattr("accessibility_toolkit.input.macos.event_tap.threading.Event", FakeEvent)
    manager = MacOSEventTapManager(
        permissions=FakePermissions(),
        backend=backend,
    )
